...
LibraryComputer::LibraryComputer()
   ...
   , style(Krampus24::Gameplay::Entities::LibraryComputer::STYLE_NORMAL)
   , uv_offset_x(0.0f)
   , uv_offset_y(0.0f)
   , initialized(false)
{
...
uint32_t LibraryComputer::get_state() const
{
   return state;
}


Krampus24::Gameplay::Entities::LibraryComputer::Style LibraryComputer::get_style() const
{
   return style;
}


float LibraryComputer::get_uv_offset_x() const
{
   return uv_offset_x;
}


float LibraryComputer::get_uv_offset_y() const
{
   return uv_offset_y;
}


void LibraryComputer::transform_model(AllegroFlare::Model3D* model, ALLEGRO_TRANSFORM* transform)
{
...
float LibraryComputer::get_random_rotation()
{
...
std::vector<Krampus24::Gameplay::Entities::Base*> LibraryComputer::construct(AllegroFlare::ModelBin* model_bin, AllegroFlare::BitmapBin* bitmap_bin, AllegroFlare::EventEmitter* event_emitter, AllegroFlare::Physics::CollisionMesh* collision_mesh, std::string name_for_collision_faces, AllegroFlare::Vec3D initial_position, float rotation)
{
   if (!(model_bin))
   {
      std::stringstream error_message;
      error_message << "[Krampus24::Gameplay::Entities::LibraryComputer::construct]: error: guard \"model_bin\" not met.";
      std::cerr << "\033[1;31m" << error_message.str() << " An exception will be thrown to halt the program.\033[0m" << std::endl;
      throw std::runtime_error("[Krampus24::Gameplay::Entities::LibraryComputer::construct]: error: guard \"model_bin\" not met");
   }
   if (!(bitmap_bin))
   {
      std::stringstream error_message;
      error_message << "[Krampus24::Gameplay::Entities::LibraryComputer::construct]: error: guard \"bitmap_bin\" not met.";
      std::cerr << "\033[1;31m" << error_message.str() << " An exception will be thrown to halt the program.\033[0m" << std::endl;
      throw std::runtime_error("[Krampus24::Gameplay::Entities::LibraryComputer::construct]: error: guard \"bitmap_bin\" not met");
   }
   if (!(event_emitter))
   {
      std::stringstream error_message;
      error_message << "[Krampus24::Gameplay::Entities::LibraryComputer::construct]: error: guard \"event_emitter\" not met.";
      std::cerr << "\033[1;31m" << error_message.str() << " An exception will be thrown to halt the program.\033[0m" << std::endl;
      throw std::runtime_error("[Krampus24::Gameplay::Entities::LibraryComputer::construct]: error: guard \"event_emitter\" not met");
   }
   if (!(collision_mesh))
   {
      std::stringstream error_message;
      error_message << "[Krampus24::Gameplay::Entities::LibraryComputer::construct]: error: guard \"collision_mesh\" not met.";
      std::cerr << "\033[1;31m" << error_message.str() << " An exception will be thrown to halt the program.\033[0m" << std::endl;
      throw std::runtime_error("[Krampus24::Gameplay::Entities::LibraryComputer::construct]: error: guard \"collision_mesh\" not met");
   }
   // Main entity
   Krampus24::Gameplay::Entities::LibraryComputer* result = new Krampus24::Gameplay::Entities::LibraryComputer;
   ...
   result->placement.size = { 3.0, 3.5, 3.0 };
   ...
   result->door->model = model_bin->auto_get("library_computer-01-body.obj");
   ...
   std::string collision_mesh_name = "library_computer-01-collision_mesh.obj";
...
void LibraryComputer::make_non_inspectable()
{
   player_can_inspect_or_use = false;
   return;
}

void LibraryComputer::unlock()
{
   locked = false;
   return;
}

void LibraryComputer::lock()
{
   locked = true;
   return;
}

void LibraryComputer::attempt_to_open()
{
   if (locked)
   {
...
void LibraryComputer::attempt_to_close()
{
   if (locked)
   {
      // TODO: Show or indicate in some way that the door is locked
...
bool LibraryComputer::on_player_inspect_or_use()
{
...
void LibraryComputer::set_style(Krampus24::Gameplay::Entities::LibraryComputer::Style style)
{
   this->style = style;
...
void LibraryComputer::set_uv_offset_x(float uv_offset_x)
{
...
void LibraryComputer::set_uv_offset_y(float uv_offset_y)
{
   this->style = STYLE_USER_CUSTOM_DEFINED_UV;
...
std::pair<float, float> LibraryComputer::get_uv_offset_from_style(Krampus24::Gameplay::Entities::LibraryComputer::Style style)
{
   if (!((style != STYLE_UNDEF)))
   {
      std::stringstream error_message;
      error_message << "[Krampus24::Gameplay::Entities::LibraryComputer::get_uv_offset_from_style]: error: guard \"(style != STYLE_UNDEF)\" not met.";
      std::cerr << "\033[1;31m" << error_message.str() << " An exception will be thrown to halt the program.\033[0m" << std::endl;
      throw std::runtime_error("[Krampus24::Gameplay::Entities::LibraryComputer::get_uv_offset_from_style]: error: guard \"(style != STYLE_UNDEF)\" not met");
   }
   switch (style)
   {
...
void LibraryComputer::set_open_position(float open_position)
{
...
void LibraryComputer::on_enter_player_bbox_collision(Krampus24::Gameplay::Entities::Base* player_entity)
{
   //attempt_to_open();
   //set_state(STATE_OPENING);
   //attempt_to_open
...
void LibraryComputer::on_exit_player_bbox_collision(Krampus24::Gameplay::Entities::Base* player_entity)
{
   //attempt_to_close();
   //set_state(STATE_CLOSING);
   return;
...
void LibraryComputer::on_time_step(double time_step, double time_now)
{
   if (!(initialized))
   {
      std::stringstream error_message;
      error_message << "[Krampus24::Gameplay::Entities::LibraryComputer::on_time_step]: error: guard \"initialized\" not met.";
      std::cerr << "\033[1;31m" << error_message.str() << " An exception will be thrown to halt the program.\033[0m" << std::endl;
      throw std::runtime_error("[Krampus24::Gameplay::Entities::LibraryComputer::on_time_step]: error: guard \"initialized\" not met");
   }
   update_state(time_step, time_now);
   return;
}

void LibraryComputer::play_open_door_sound_effect()
{
...
std::map<std::string, AllegroFlare::AudioRepositoryElement> LibraryComputer::build_audio_controller_sound_effect_list()
{
...
void LibraryComputer::set_state(uint32_t state, bool override_if_busy)
{
   if (!(initialized))
   {
      std::stringstream error_message;
      error_message << "[Krampus24::Gameplay::Entities::LibraryComputer::set_state]: error: guard \"initialized\" not met.";
      std::cerr << "\033[1;31m" << error_message.str() << " An exception will be thrown to halt the program.\033[0m" << std::endl;
      throw std::runtime_error("[Krampus24::Gameplay::Entities::LibraryComputer::set_state]: error: guard \"initialized\" not met");
   }
   if (!(is_valid_state(state)))
   {
      std::stringstream error_message;
      error_message << "[Krampus24::Gameplay::Entities::LibraryComputer::set_state]: error: guard \"is_valid_state(state)\" not met.";
      std::cerr << "\033[1;31m" << error_message.str() << " An exception will be thrown to halt the program.\033[0m" << std::endl;
      throw std::runtime_error("[Krampus24::Gameplay::Entities::LibraryComputer::set_state]: error: guard \"is_valid_state(state)\" not met");
   }
...
void LibraryComputer::update_state(double time_step, double time_now)
{
   if (!(initialized))
   {
      std::stringstream error_message;
      error_message << "[Krampus24::Gameplay::Entities::LibraryComputer::update_state]: error: guard \"initialized\" not met.";
      std::cerr << "\033[1;31m" << error_message.str() << " An exception will be thrown to halt the program.\033[0m" << std::endl;
      throw std::runtime_error("[Krampus24::Gameplay::Entities::LibraryComputer::update_state]: error: guard \"initialized\" not met");
   }
   if (!(is_valid_state(state)))
   {
      std::stringstream error_message;
      error_message << "[Krampus24::Gameplay::Entities::LibraryComputer::update_state]: error: guard \"is_valid_state(state)\" not met.";
      std::cerr << "\033[1;31m" << error_message.str() << " An exception will be thrown to halt the program.\033[0m" << std::endl;
      throw std::runtime_error("[Krampus24::Gameplay::Entities::LibraryComputer::update_state]: error: guard \"is_valid_state(state)\" not met");
   }
...
bool LibraryComputer::is_valid_state(uint32_t state)
{
...
bool LibraryComputer::is_state(uint32_t possible_state)
{
   return (state == possible_state);
}

float LibraryComputer::infer_current_state_age(float time_now)
{
   if (!(initialized))
   {
      std::stringstream error_message;
      error_message << "[Krampus24::Gameplay::Entities::LibraryComputer::infer_current_state_age]: error: guard \"initialized\" not met.";
      std::cerr << "\033[1;31m" << error_message.str() << " An exception will be thrown to halt the program.\033[0m" << std::endl;
      throw std::runtime_error("[Krampus24::Gameplay::Entities::LibraryComputer::infer_current_state_age]: error: guard \"initialized\" not met");
   }
