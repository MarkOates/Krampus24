...
WideTable::WideTable()
   ...
   , style(Krampus24::Gameplay::Entities::WideTable::STYLE_NORMAL)
   , uv_offset_x(0.0f)
   , uv_offset_y(0.0f)
   , initialized(false)
{
...
uint32_t WideTable::get_state() const
{
   return state;
}


Krampus24::Gameplay::Entities::WideTable::Style WideTable::get_style() const
{
   return style;
}


float WideTable::get_uv_offset_x() const
{
   return uv_offset_x;
}


float WideTable::get_uv_offset_y() const
{
   return uv_offset_y;
}


void WideTable::transform_model(AllegroFlare::Model3D* model, ALLEGRO_TRANSFORM* transform)
{
...
float WideTable::get_random_rotation()
{
...
std::vector<Krampus24::Gameplay::Entities::Base*> WideTable::construct(AllegroFlare::ModelBin* model_bin, AllegroFlare::BitmapBin* bitmap_bin, AllegroFlare::EventEmitter* event_emitter, AllegroFlare::Physics::CollisionMesh* collision_mesh, std::string name_for_collision_faces, AllegroFlare::Vec3D initial_position, float rotation)
{
   if (!(model_bin))
   {
      std::stringstream error_message;
      error_message << "[Krampus24::Gameplay::Entities::WideTable::construct]: error: guard \"model_bin\" not met.";
      std::cerr << "\033[1;31m" << error_message.str() << " An exception will be thrown to halt the program.\033[0m" << std::endl;
      throw std::runtime_error("[Krampus24::Gameplay::Entities::WideTable::construct]: error: guard \"model_bin\" not met");
   }
   if (!(bitmap_bin))
   {
      std::stringstream error_message;
      error_message << "[Krampus24::Gameplay::Entities::WideTable::construct]: error: guard \"bitmap_bin\" not met.";
      std::cerr << "\033[1;31m" << error_message.str() << " An exception will be thrown to halt the program.\033[0m" << std::endl;
      throw std::runtime_error("[Krampus24::Gameplay::Entities::WideTable::construct]: error: guard \"bitmap_bin\" not met");
   }
   if (!(event_emitter))
   {
      std::stringstream error_message;
      error_message << "[Krampus24::Gameplay::Entities::WideTable::construct]: error: guard \"event_emitter\" not met.";
      std::cerr << "\033[1;31m" << error_message.str() << " An exception will be thrown to halt the program.\033[0m" << std::endl;
      throw std::runtime_error("[Krampus24::Gameplay::Entities::WideTable::construct]: error: guard \"event_emitter\" not met");
   }
   if (!(collision_mesh))
   {
      std::stringstream error_message;
      error_message << "[Krampus24::Gameplay::Entities::WideTable::construct]: error: guard \"collision_mesh\" not met.";
      std::cerr << "\033[1;31m" << error_message.str() << " An exception will be thrown to halt the program.\033[0m" << std::endl;
      throw std::runtime_error("[Krampus24::Gameplay::Entities::WideTable::construct]: error: guard \"collision_mesh\" not met");
   }
   // Main entity
   Krampus24::Gameplay::Entities::WideTable* result = new Krampus24::Gameplay::Entities::WideTable;
   ...
   result->placement.size = { 4.0, 2.0, 4.0 };
   result->aabb3d.set_max(result->placement.size);
   result->aabb3d_alignment = { 0.5, 0.01, 0.5 }; // Just slightly below the floor
   result->initial_position = initial_position;
   result->placement.rotation.y = rotation; //get_random_rotation();
   result->player_can_inspect_or_use = false; // By default, player cannot inspect.  In some cases, items may be
                                              // placed on the table, and right now, it's easier to inspect the
                                              // "table" than the object with the current system.
   result->locked = false;

   // Left door
   result->door = new Krampus24::Gameplay::Entities::Base;
   result->door->model = model_bin->auto_get("assorted_props-01-wide_table.obj");
   ...
   std::string collision_mesh_name = "assorted_props-01-wide_table-collision_mesh.obj";
...
void WideTable::make_inspectable()
{
   player_can_inspect_or_use = true;
   return;
}

void WideTable::make_non_inspectable()
{
   player_can_inspect_or_use = false;
   return;
}

void WideTable::unlock()
{
   locked = false;
   return;
}
...
void WideTable::attempt_to_open()
{
   if (locked)
   {
...
void WideTable::attempt_to_close()
{
   if (locked)
   {
      // TODO: Show or indicate in some way that the door is locked
...
bool WideTable::on_player_inspect_or_use()
{
...
void WideTable::set_style(Krampus24::Gameplay::Entities::WideTable::Style style)
{
   this->style = style;
...
void WideTable::set_uv_offset_x(float uv_offset_x)
{
...
void WideTable::set_uv_offset_y(float uv_offset_y)
{
   this->style = STYLE_USER_CUSTOM_DEFINED_UV;
...
std::pair<float, float> WideTable::get_uv_offset_from_style(Krampus24::Gameplay::Entities::WideTable::Style style)
{
   if (!((style != STYLE_UNDEF)))
   {
      std::stringstream error_message;
      error_message << "[Krampus24::Gameplay::Entities::WideTable::get_uv_offset_from_style]: error: guard \"(style != STYLE_UNDEF)\" not met.";
      std::cerr << "\033[1;31m" << error_message.str() << " An exception will be thrown to halt the program.\033[0m" << std::endl;
      throw std::runtime_error("[Krampus24::Gameplay::Entities::WideTable::get_uv_offset_from_style]: error: guard \"(style != STYLE_UNDEF)\" not met");
   }
   switch (style)
   {
...
void WideTable::set_open_position(float open_position)
{
...
void WideTable::on_enter_player_bbox_collision(Krampus24::Gameplay::Entities::Base* player_entity)
{
   //attempt_to_open();
   //set_state(STATE_OPENING);
   //attempt_to_open
...
void WideTable::on_exit_player_bbox_collision(Krampus24::Gameplay::Entities::Base* player_entity)
{
   //attempt_to_close();
   //set_state(STATE_CLOSING);
   return;
...
void WideTable::on_time_step(double time_step, double time_now)
{
   if (!(initialized))
   {
      std::stringstream error_message;
      error_message << "[Krampus24::Gameplay::Entities::WideTable::on_time_step]: error: guard \"initialized\" not met.";
      std::cerr << "\033[1;31m" << error_message.str() << " An exception will be thrown to halt the program.\033[0m" << std::endl;
      throw std::runtime_error("[Krampus24::Gameplay::Entities::WideTable::on_time_step]: error: guard \"initialized\" not met");
   }
   update_state(time_step, time_now);
   return;
}

void WideTable::play_open_door_sound_effect()
{
...
std::map<std::string, AllegroFlare::AudioRepositoryElement> WideTable::build_audio_controller_sound_effect_list()
{
...
void WideTable::set_state(uint32_t state, bool override_if_busy)
{
   if (!(initialized))
   {
      std::stringstream error_message;
      error_message << "[Krampus24::Gameplay::Entities::WideTable::set_state]: error: guard \"initialized\" not met.";
      std::cerr << "\033[1;31m" << error_message.str() << " An exception will be thrown to halt the program.\033[0m" << std::endl;
      throw std::runtime_error("[Krampus24::Gameplay::Entities::WideTable::set_state]: error: guard \"initialized\" not met");
   }
   if (!(is_valid_state(state)))
   {
      std::stringstream error_message;
      error_message << "[Krampus24::Gameplay::Entities::WideTable::set_state]: error: guard \"is_valid_state(state)\" not met.";
      std::cerr << "\033[1;31m" << error_message.str() << " An exception will be thrown to halt the program.\033[0m" << std::endl;
      throw std::runtime_error("[Krampus24::Gameplay::Entities::WideTable::set_state]: error: guard \"is_valid_state(state)\" not met");
   }
...
void WideTable::update_state(double time_step, double time_now)
{
   if (!(initialized))
   {
      std::stringstream error_message;
      error_message << "[Krampus24::Gameplay::Entities::WideTable::update_state]: error: guard \"initialized\" not met.";
      std::cerr << "\033[1;31m" << error_message.str() << " An exception will be thrown to halt the program.\033[0m" << std::endl;
      throw std::runtime_error("[Krampus24::Gameplay::Entities::WideTable::update_state]: error: guard \"initialized\" not met");
   }
   if (!(is_valid_state(state)))
   {
      std::stringstream error_message;
      error_message << "[Krampus24::Gameplay::Entities::WideTable::update_state]: error: guard \"is_valid_state(state)\" not met.";
      std::cerr << "\033[1;31m" << error_message.str() << " An exception will be thrown to halt the program.\033[0m" << std::endl;
      throw std::runtime_error("[Krampus24::Gameplay::Entities::WideTable::update_state]: error: guard \"is_valid_state(state)\" not met");
   }
...
bool WideTable::is_valid_state(uint32_t state)
{
...
bool WideTable::is_state(uint32_t possible_state)
{
   return (state == possible_state);
}

float WideTable::infer_current_state_age(float time_now)
{
   if (!(initialized))
   {
      std::stringstream error_message;
      error_message << "[Krampus24::Gameplay::Entities::WideTable::infer_current_state_age]: error: guard \"initialized\" not met.";
      std::cerr << "\033[1;31m" << error_message.str() << " An exception will be thrown to halt the program.\033[0m" << std::endl;
      throw std::runtime_error("[Krampus24::Gameplay::Entities::WideTable::infer_current_state_age]: error: guard \"initialized\" not met");
   }
